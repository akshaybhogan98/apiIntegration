...
public class HourlyForecastController {
    ...
    @GetMapping(value="v1/hourlyForecastSummary/location/{location}",produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponsePayload<ResponseDto> getForecastSumaryByLocation(@PathVariable("location") String location){
        final ResponseDto responsePayload=forecastByLocationService.fetchHourlyForecastByLocation(location);
